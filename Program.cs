...
";
        ...
        static void DrawSettingsTab()
        {
            ...
            if (config.AutoUnfriendEnabled)
            {
                ...
                bool limitTrigger = config.FriendLimitTriggerEnabled;
                if (ImGui.Checkbox("Trigger immediately when friend limit reached", ref limitTrigger))
                {
                    config.FriendLimitTriggerEnabled = limitTrigger;
                    SaveConfig();
                }

                if (config.FriendLimitTriggerEnabled)
                {
                    ImGui.Spacing();
                    ImGui.Text("Trigger at:");
                    ImGui.SameLine();
                    int threshold = config.FriendLimitThreshold;
                    ImGui.SetNextItemWidth(80);
                    if (ImGui.InputInt("##flthresh", ref threshold, 1, 10))
                    {
                        config.FriendLimitThreshold = Math.Clamp(threshold, 1, 1000);
                        SaveConfig();
                    }
                    ImGui.SameLine();
                    ...
                    int cur = friends.Count;
                    float ratio = config.FriendLimitThreshold > 0
                        ? Math.Min(1f, cur / (float)config.FriendLimitThreshold) : 0f;
                    var barCol = ratio >= 1f
                        ? new Vector4(1f, 0.3f, 0.2f, 1f)
                        : ratio >= 0.9f
                            ? new Vector4(0.9f, 0.7f, 0.1f, 1f)
                    ...
                    ImGui.PopStyleColor();
                    ImGui.SameLine();
                    ImGui.TextColored(barCol, $"{cur} / {config.FriendLimitThreshold}");

                    if (ratio >= 1f)
                        ImGui.TextColored(new Vector4(1f, 0.4f, 0.3f, 1f), "  ● At or above threshold — will trigger on next check (every 2 min)");
                    else
                        ImGui.TextDisabled($"  Checked every 2 minutes");
                }
            }
        }
...
".Trim();
        ...
        static void StartAutoScheduler()
        {
            ...
            // Polls at the user-configured interval. If the friend count hits the
            // configured threshold it fires RunAutoUnfriendAsync immediately,
            // bypassing the normal schedule.
            _ = Task.Run(async () =>
            {
                while (!token.IsCancellationRequested && config.AutoUnfriendEnabled)
                {
                    int intervalMin = Math.Clamp(config.FriendLimitPollIntervalMinutes, 1, 60);
                    try { await Task.Delay(TimeSpan.FromMinutes(intervalMin), token); }
                    catch (OperationCanceledException) { break; }

                    if (token.IsCancellationRequested) break;
                    if (!config.FriendLimitTriggerEnabled) continue;

                    int count = friends.Count;
                    if (count >= config.FriendLimitThreshold)
                    {
                        Console.WriteLine($"[LIMIT-TRIGGER] Friend count {count} >= threshold {config.FriendLimitThreshold}, firing auto-unfriend");
                        ShowToast("Friend Limit Reached", $"{count} friends — running auto-unfriend now.");
                        await RunAutoUnfriendAsync(token);
                    }
                }
            }, token);
